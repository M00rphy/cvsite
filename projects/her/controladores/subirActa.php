...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($_FILES['ebook']['error'] !== 0) {
        ...
        if (
            ...
            $ruta_ebook = '../solvencias/' . $expediente . '-' . $usuario . '-' . '.pdf';
            $ruta = '/solvencias/' . $expediente . '-' . $usuario . '-' . '.pdf';
            if (move_uploaded_file($ebook, $ruta_ebook)) {

                $consultaDocumento = "INSERT INTO actas (expediente, usuario, ruta, idTipoParticipacion)
